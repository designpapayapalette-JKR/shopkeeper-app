  ...
  return (
    <View>
      <View className="flex-row flex-wrap" style={{ gap: 8 }}>
        {GST_SLABS.map((slab) => {
          ...
          return (
            ...
            >
              <Text className={`text-sm font-bold ${isActive ? "text-white" : "text-on-surface dark:text-text-primary-dark"}`}>
                {slab}%
              </Text>
            </Pressable>
          );
      ...
      <TextInput
        ...
        className="bg-surface-container-lowest dark:bg-surface-dark text-on-surface dark:text-text-primary-dark border border-outline-variant dark:border-outline rounded-xl px-4 py-4 text-base font-medium mt-2.5"
      />
    </View>
  );
}
