  ...
  if (def.data_type === "boolean") {
    ...
    return (
      <View className="flex-row" style={{ gap: 8 }}>
        ...
        >
          <Text
            className={`text-sm font-bold ${boolVal ? "text-white" : "text-on-surface dark:text-text-primary-dark"}`}
          >
            Yes
          </Text>
        </Pressable>
            ...
            >
              <Text
                className={`text-sm font-semibold ${
                  isActive ? "text-white" : "text-on-surface dark:text-text-primary-dark"
                }`}
              >
                {c}
              </Text>
  ...
  if (def.data_type === "number" || def.data_type === "decimal") {
    ...
    return (
      <TextInput
        ...
        className="bg-surface-container-lowest dark:bg-surface-dark text-on-surface dark:text-text-primary-dark border border-outline-variant dark:border-outline rounded-xl px-4 py-3 text-base font-medium"
      />
    );
  }
    ...
    return (
      <View>
        <TextInput
          ...
          placeholder="Value"
          placeholderTextColor="#A0A0A0"
          className="bg-surface-container-lowest dark:bg-surface-dark text-on-surface dark:text-text-primary-dark border border-outline-variant dark:border-outline rounded-xl px-4 py-3 text-base font-medium mb-2"
        />
        <View className="flex-row flex-wrap" style={{ gap: 6 }}>
        ...
        <View className="flex-row" style={{ gap: 8 }}>
          {(["l", "w", "h"] as const).map((dim) => (
            <View key={dim} className="flex-1">
              <Text className="text-xs font-semibold text-on-surface-variant dark:text-text-secondary-dark mb-1 text-center">
                {dim.toUpperCase()}
              </Text>
              <TextInput
                ...
                placeholder="0"
                placeholderTextColor="#A0A0A0"
                className="bg-surface-container-lowest dark:bg-surface-dark text-on-surface dark:text-text-primary-dark border border-outline-variant dark:border-outline rounded-xl px-3 py-3 text-base font-medium text-center"
              />
            </View>
          ))}
        </View>
    ...
    <View className="mt-6 pt-4 border-t border-outline-variant dark:border-outline">
      {Object.entries(grouped).map(([group, groupDefs]) => (
        <View key={group} className="mb-4">
          <Text className="text-xs font-black uppercase tracking-wider text-on-surface-variant dark:text-text-secondary-dark mb-3">
            {group}
          </Text>
          <View style={{ gap: 12 }}>
            {groupDefs.map((def) => (
              <View key={def.id}>
                <Text className="text-sm font-semibold text-on-surface dark:text-text-primary-dark mb-1.5">
                  {def.label}
                </Text>
                <ValueInput
                  def={def}
