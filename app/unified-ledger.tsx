...
export default function UnifiedLedgerScreen() {
 ...
 partyFilter === f ? "bg-primary " : "bg-transparent"
 }`}
 >
 <Text
 className={`text-sm font-bold capitalize ${
 partyFilter === f ? "text-white" : "text-on-surface-variant "
 }`}
 >
 {f === "all" ? "All" : `${f}s`}
 </Text>
 ...
 ListFooterComponent={loadingMore ? <View className="py-4"><ActivityIndicator size="small" color={theme.colors.primary} /></View> : !hasMore && entries.length > 0 ? <View className="py-4"><Text className="text-center text-sm text-on-surface-variant ">All entries loaded</Text></View> : null}
 ...
 <Text className="text-xs font-bold text-on-surface-variant mt-0.5 uppercase tracking-widest">
 {item.party.type}
 </Text>
 </View>
 </Pressable>
