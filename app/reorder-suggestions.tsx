    ...
    <ScrollView style={{ flex: 1 }}>
      <View className="px-4 py-3">
        <View className="flex-row justify-between items-center mb-2">
          <Text className="text-xl font-black text-on-surface dark:text-text-primary-dark">Reorder Suggestions</Text>
          {!loading && (
        ...
        {loading ? (
          ...
          <>
            <View className="flex-row gap-2 mb-4">
              <Pressable onPress={selectAll} className="bg-surface-container-lowest dark:bg-surface-dark border border-outline-variant dark:border-outline px-4 py-2 rounded-xl">
              ...
              <Pressable
                ...
                className="bg-primary dark:bg-primary-dark px-4 py-2 rounded-xl"
              >
            ...
            {suggestions.map((s) => (
              <Pressable
                ...
                className="bg-surface-container-lowest dark:bg-surface-dark rounded-xl border border-outline-variant dark:border-outline p-4 mb-2 flex-row items-center"
              >
                ...
                <View className="flex-1">
                  <Text className="text-sm font-bold text-on-surface dark:text-text-primary-dark">{s.product_name}</Text>
      ...
      <Modal visible={showSupplierModal} transparent animationType="slide">
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.5)" }}>
          <View className="mt-auto bg-surface-container-lowest dark:bg-surface-dark rounded-t-3xl p-6 max-h-[80%]">
            <Text className="text-lg font-black mb-2 text-on-surface dark:text-text-primary-dark">Select Supplier</Text>
            <Text className="text-sm text-on-surface-variant dark:text-text-secondary-dark mb-4">
              Choose a supplier for the purchase order.
            </Text>

            {suppliers.length === 0 ? (
              <Text className="text-sm text-on-surface-variant dark:text-text-secondary-dark text-center py-4">
                  ...
                  <Pressable
                    ...
                    className={`p-4 rounded-xl mb-2 border ${
                      selectedSupplier?.id === s.id
                        ...
                        : "border-outline-variant dark:border-outline bg-surface-container dark:bg-zinc-800"
                    }`}
                  >
                    <Text className="font-bold text-sm text-on-surface dark:text-text-primary-dark">{s.name}</Text>
                    {s.phone && <Text className="text-xs text-on-surface-variant dark:text-text-secondary-dark">{s.phone}</Text>}
                  </Pressable>
                ))}
              </ScrollView>
            )}
            ...
            <TextInput
              ...
              className="bg-surface-container dark:bg-zinc-800 border border-outline-variant dark:border-outline rounded-xl px-4 py-3 mb-4 text-sm text-on-surface dark:text-text-primary-dark"
              value={poNotes}
            ...
            <View className="flex-row gap-3">
              <Pressable
                ...
                className="flex-1 bg-surface-container dark:bg-zinc-800 py-3 rounded-xl items-center"
              >
                <Text className="font-bold text-on-surface dark:text-text-primary-dark">Cancel</Text>
              </Pressable>
              <Pressable
                onPress={generatePO}
                disabled={!selectedSupplier || generating}
                className="flex-1 bg-primary dark:bg-primary-dark py-3 rounded-xl items-center"
              >
