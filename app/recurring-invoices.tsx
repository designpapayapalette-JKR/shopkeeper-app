...
export default function RecurringInvoicesScreen() {
 ...
 <Text className="text-xl font-black text-on-surface ">Recurring Invoices</Text>
 <Text className="text-sm text-on-surface-variant mt-0.5">Auto-generate on a repeating schedule.</Text>
 </View>
 </View>
 ...
 }} className="bg-surface-container-lowest p-4 rounded-xl border border-outline-variant mb-3">
 <View className="flex-row justify-between items-center mb-2">
 <Text className="font-bold text-on-surface flex-1 mr-2" numberOfLines={1}>{t.party?.name || "—"}</Text>
 <Pressable onPress={() => toggleActive(t)} className={`px-3 py-2 rounded-lg ${t.is_active ? "bg-success/10" : "bg-surface-container "}`}>
 <Text className={`text-xs font-bold ${t.is_active ? "text-success" : "text-on-surface-variant "}`}>{t.is_active ? "Active" : "Paused"}</Text>
 </Pressable>
 </View>
 <View className="flex-row items-center gap-2">
 <Text className="text-xs text-on-surface-variant capitalize">{t.frequency}</Text>
 <Text className="text-xs text-on-surface-variant ">·</Text>
 <Text className="text-xs text-on-surface-variant ">next {new Date(t.next_run_date).toLocaleDateString("en-IN")}</Text>
 <Text className="text-xs text-on-surface-variant ">·</Text>
 <Text className="text-xs text-on-surface-variant ">{totalItems(t.items)} items</Text>
 </View>
 </Pressable>
 ))
 )}
 ...
 {parties.map((p) => (
 <Pressable key={p.id} onPress={() => setPartyId(p.id)} className={`mr-2 px-4 py-3 rounded-lg border ${partyId === p.id ? "bg-primary border-primary" : "bg-surface-container-lowest border-outline-variant "}`}>
 <Text className={`text-sm font-semibold ${partyId === p.id ? "text-white" : "text-on-surface-variant "}`}>{p.name}</Text>
 </Pressable>
 ))}
 </ScrollView>

 <Text className="text-sm font-semibold text-on-surface-variant uppercase tracking-wider mb-2">Frequency</Text>
 <View className="flex-row flex-wrap mb-4" style={{ gap: 8 }}>
 {FREQUENCIES.map((f) => (
 <Pressable key={f} onPress={() => setFrequency(f)} className={`px-4 py-2.5 rounded-lg border ${frequency === f ? "bg-primary border-primary" : "bg-surface-container-lowest border-outline-variant "}`}>
 <Text className={`text-sm font-semibold capitalize ${frequency === f ? "text-white" : "text-on-surface-variant "}`}>{f}</Text>
 </Pressable>
 ))}
 </View>

 <Text className="text-sm font-semibold text-on-surface-variant uppercase tracking-wider mb-2">Next Run Date</Text>
 <Pressable onPress={() => setShowDatePicker(true)} className="bg-surface-container-lowest border border-outline-variant rounded-xl px-4 py-3 mb-4 flex-row items-center justify-between">
 <Text className="text-base font-bold text-on-surface ">{nextRunDate}</Text>
 <MaterialCommunityIcons name="calendar" size={20} color={theme.colors.primary} />
 </Pressable>

 <Text className="text-sm font-semibold text-on-surface-variant uppercase tracking-wider mb-2">Products ({cart.length})</Text>
 {cart.map((item, idx) => (
 <View key={item.productId} className="bg-surface-container-lowest border border-outline-variant rounded-xl p-3 mb-2 flex-row items-center">
 <View className="flex-1 mr-2">
 <Text className="text-sm font-bold text-on-surface " numberOfLines={1}>{item.name}</Text>
 <Text className="text-xs text-on-surface-variant mt-0.5">₹{item.price.toLocaleString("en-IN")} each</Text>
 </View>
 <View className="flex-row items-center gap-2">
 <Pressable onPress={() => updateCartQty(idx, item.quantity - 1)} className="w-8 h-8 rounded-full bg-surface-container items-center justify-center">
 <MaterialCommunityIcons name="minus" size={16} color={theme.colors.onSurfaceVariant} />
 </Pressable>
 <Text className="text-sm font-bold text-on-surface w-6 text-center">{item.quantity}</Text>
 <Pressable onPress={() => updateCartQty(idx, item.quantity + 1)} className="w-8 h-8 rounded-full bg-surface-container items-center justify-center">
 ...
 <Pressable onPress={() => setShowProductPicker(true)} className="bg-surface-container-lowest border border-dashed border-outline-variant rounded-xl p-3 mb-4 items-center">
 ...
 <Pressable className="bg-background rounded-2xl p-4 max-h-96" onPress={() => {}}>
 <Text className="text-lg font-bold text-on-surface mb-3">Select Product</Text>
 <ScrollView>
 {products.slice(0, 50).map((p) => (
 <Pressable key={p.id} onPress={() => addProductToCart(p)} className="py-3 border-b border-outline-variant flex-row justify-between">
 <Text className="text-sm font-semibold text-on-surface flex-1 mr-2" numberOfLines={1}>{p.name}</Text>
 <Text className="text-sm font-bold text-primary">₹{p.price}</Text>
 </Pressable>
 ))}
 {products.length === 0 && (
 <Text className="text-sm text-on-surface-variant text-center py-4">No products found.</Text>
 )}
 </ScrollView>
 </Pressable>
 </Pressable>
