...
export default function PriceListsScreen() {
 ...
 <Text className="text-xs text-on-surface-variant mt-0.5 capitalize">{item.type}</Text>
 </View>
 ...
 <Text className="text-xl font-bold text-on-surface ">Price Lists</Text>
 </View>
 ...
 className="bg-surface-container-lowest text-on-surface border border-outline-variant rounded-xl px-4 py-3.5 font-medium mb-5" />

 <Text className="text-sm font-semibold text-on-surface-variant uppercase tracking-wider mb-2">Type</Text>
 <View className="flex-row" style={{ gap: 8 }}>
 {LIST_TYPES.map((t) => (
 <Pressable key={t.id} onPress={() => setFormType(t.id)}
 className={`px-4 py-3 rounded-xl border ${formType === t.id ? "bg-primary border-primary" : "bg-surface-container-lowest border-outline-variant "}`}>
 <Text className={`text-sm font-bold ${formType === t.id ? "text-white" : "text-on-surface-variant "}`}>{t.label}</Text>
 </Pressable>
 ))}
 </View>
 ...
 <Text className="text-sm font-bold text-on-surface ">Set as default</Text>
 </Pressable>
 ...
 <Text className="text-xl font-bold text-on-surface ">{activeList?.name}</Text>
 </View>
 ...
 <Text className="text-sm font-bold text-on-surface-variant ">No items yet. Add products to this price list.</Text>
 </View>
 ) : (
 ...
 renderItem={({ item }) => (
 <View className="bg-surface-container-lowest p-4 rounded-2xl border border-outline-variant mb-2">
 <View className="flex-row items-center">
 <View className="flex-1 mr-2">
 <Text className="text-sm font-bold text-on-surface ">{item.product.name}</Text>
 <Text className="text-xs text-on-surface-variant ">{item.product.sku || "No SKU"}</Text>
 </View>
 <TextInput
 value={item.unit_price.toString()}
 onChangeText={(v) => handleUpdateItemPrice(item.id, v)}
 keyboardType="decimal-pad"
 className="bg-background text-on-surface border border-outline-variant rounded-lg px-3 py-2 text-sm font-bold w-24 text-right mr-2" />
 ...
 <Text className="text-xs text-on-surface-variant mt-1">Min qty: {item.min_quantity} · Regular: ₹{Number(item.product.price).toLocaleString("en-IN")}</Text>
 </View>
 )} />
 )}
 </View>
 ...
 <View className="flex-row justify-between items-center mb-6">
 <Text className="text-2xl font-bold text-on-surface ">Add Item</Text>
 ...
 className={`px-3.5 py-2.5 rounded-xl border ${selectedProductId === p.id ? "bg-primary border-primary" : "bg-surface-container-lowest border-outline-variant "}`}>
 <Text className={`text-sm font-bold ${selectedProductId === p.id ? "text-white" : "text-on-surface-variant "}`}>{p.name}</Text>
 </Pressable>
 ))}
 </View>

 <Text className="text-sm font-semibold text-on-surface-variant uppercase tracking-wider mb-2">Unit Price *</Text>
 ...
 <Text className="text-sm font-semibold text-on-surface-variant uppercase tracking-wider mb-2">Min Quantity</Text>
 <TextInput value={itemMinQty} onChangeText={setItemMinQty} keyboardType="numeric"
 className="bg-surface-container-lowest text-on-surface border border-outline-variant rounded-xl px-4 py-3.5 font-medium mb-6" />
