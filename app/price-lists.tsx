    ...
    <Pressable onPress={() => openItems(item)}
      ...
      <View className="flex-row items-start justify-between">
        <View className="flex-1 mr-2">
          ...
          <Text className="text-xs text-text-secondary mt-0.5 capitalize">{item.type}</Text>
        </View>
    ...
    <View className="flex-1 bg-background dark:bg-background-dark" style={{ paddingTop: topInset }}>
      <View className="flex-row items-center justify-between px-6 py-4">
        <View className="flex-row items-center" style={{ gap: 8 }}>
          ...
          <Text className="text-xl font-bold text-text-primary dark:text-text-primary-dark">Price Lists</Text>
        </View>
      ...
      {loading ? (
        ...
        <View className="flex-1 items-center justify-center pb-20 px-6">
          <MaterialCommunityIcons name="tag-outline" size={48} color="#D1D5DB" />
          <Text className="text-base font-bold text-text-secondary mt-4">No price lists yet</Text>
          <Text className="text-sm text-text-secondary mt-1 text-center">Create price lists to offer different pricing tiers.</Text>
        </View>
      ) : (
          ...
          <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} className="flex-1">
            <ScrollView className="flex-1 bg-background dark:bg-background-dark px-6 pb-10" style={{ paddingTop: topInset }}>
              ...
              <TextInput value={formName} onChangeText={setFormName} placeholder="e.g. Wholesale 2024" placeholderTextColor="#A0A0A0" autoFocus
                className="bg-surface dark:bg-zinc-900 text-text-primary border border-gray-200 dark:border-zinc-800 rounded-xl px-4 py-3.5 font-medium mb-5" />

              <Text className="text-sm font-semibold text-text-secondary uppercase tracking-wider mb-2">Type</Text>
              <View className="flex-row" style={{ gap: 8 }}>
                {LIST_TYPES.map((t) => (
                  <Pressable key={t.id} onPress={() => setFormType(t.id)}
                    className={`px-4 py-3 rounded-xl border ${formType === t.id ? "bg-primary border-primary" : "bg-surface dark:bg-zinc-900 border-gray-200 dark:border-zinc-800"}`}>
                    <Text className={`text-sm font-bold ${formType === t.id ? "text-white" : "text-text-secondary"}`}>{t.label}</Text>
                  </Pressable>
                ))}
              </View>
              ...
              <Pressable onPress={() => setFormIsDefault(!formIsDefault)}
                ...
                <Text className="text-sm font-bold text-text-primary dark:text-text-primary-dark">Set as default</Text>
              </Pressable>
          ...
          <View className="flex-1 bg-background dark:bg-background-dark" style={{ paddingTop: topInset }}>
            <View className="flex-row items-center justify-between px-6 py-4">
              <View className="flex-row items-center" style={{ gap: 8 }}>
                ...
                <Text className="text-xl font-bold text-text-primary dark:text-text-primary-dark">{activeList?.name}</Text>
              </View>
            ...
            {itemsLoading ? (
              ...
              <View className="flex-1 items-center justify-center px-6">
                <Text className="text-sm font-bold text-text-secondary">No items yet. Add products to this price list.</Text>
              </View>
            ) : (
              <FlatList data={items} keyExtractor={(item) => item.id} showsVerticalScrollIndicator={false}
                ...
                renderItem={({ item }) => (
                  <View className="bg-surface dark:bg-surface-dark p-4 rounded-2xl border border-gray-100 dark:border-zinc-800 mb-2">
                    <View className="flex-row items-center">
                      <View className="flex-1 mr-2">
                        <Text className="text-sm font-bold text-text-primary dark:text-text-primary-dark">{item.product.name}</Text>
                        <Text className="text-xs text-text-secondary">{item.product.sku || "No SKU"}</Text>
                      </View>
                      <TextInput
                        value={item.unit_price.toString()}
                        onChangeText={(v) => handleUpdateItemPrice(item.id, v)}
                        keyboardType="decimal-pad"
                        className="bg-background dark:bg-zinc-900 text-text-primary border border-gray-200 dark:border-zinc-800 rounded-lg px-3 py-2 text-sm font-bold w-24 text-right mr-2" />
                    ...
                    <Text className="text-xs text-text-secondary mt-1">Min qty: {item.min_quantity} · Regular: ₹{Number(item.product.price).toLocaleString("en-IN")}</Text>
                  </View>
                )} />
            )}
          </View>
        ...
        <SafeAreaProvider>
          <ScrollView className="flex-1 bg-background dark:bg-background-dark" style={{ paddingTop: topInset }}>
            <View className="px-6 pb-8">
              <View className="flex-row justify-between items-center mb-6">
                <Text className="text-2xl font-bold text-text-primary dark:text-text-primary-dark">Add Item</Text>
                <Pressable onPress={() => setShowAddItem(false)} className="w-11 h-11 items-center justify-center">
                  <MaterialCommunityIcons name="close" size={20} color="#6B7280" />
                </Pressable>
              </View>

              <Text className="text-sm font-semibold text-text-secondary uppercase tracking-wider mb-2">Product</Text>
              <TextInput value={productSearch} onChangeText={setProductSearch} placeholder="Search products..." placeholderTextColor="#A0A0A0"
                className="bg-surface dark:bg-zinc-900 text-text-primary border border-gray-200 dark:border-zinc-800 rounded-xl px-4 py-3.5 font-medium mb-3" />
              <View className="flex-row flex-wrap mb-5" style={{ gap: 6 }}>
                {filteredProducts.slice(0, 20).map((p) => (
                  <Pressable key={p.id} onPress={() => setSelectedProductId(p.id)}
                    className={`px-3.5 py-2.5 rounded-xl border ${selectedProductId === p.id ? "bg-primary border-primary" : "bg-surface dark:bg-zinc-900 border-gray-200 dark:border-zinc-800"}`}>
                    <Text className={`text-sm font-bold ${selectedProductId === p.id ? "text-white" : "text-text-secondary"}`}>{p.name}</Text>
                  </Pressable>
                ))}
              </View>

              <Text className="text-sm font-semibold text-text-secondary uppercase tracking-wider mb-2">Unit Price *</Text>
              ...
              <Text className="text-sm font-semibold text-text-secondary uppercase tracking-wider mb-2">Min Quantity</Text>
              <TextInput value={itemMinQty} onChangeText={setItemMinQty} keyboardType="numeric"
                className="bg-surface dark:bg-zinc-900 text-text-primary border border-gray-200 dark:border-zinc-800 rounded-xl px-4 py-3.5 font-medium mb-6" />
