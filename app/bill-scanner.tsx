...
const CATEGORY_META: Record<ScanCategory, { title: string; destination: (uri: string) => string }> = {
 ...
 destination: (uri) => `/purchase-entry?billPhotoUri=${encodeURIComponent(uri)}`,
 },
 product: {
 ...
 destination: (uri) => `/expenses?billPhotoUri=${encodeURIComponent(uri)}`,
 },
 transfer: {
 title: "Photograph the transfer receipt",
 destination: (uri) => `/stock-transfer-requests?transferPhotoUri=${encodeURIComponent(uri)}`,
 },
};
...
export default function BillScannerScreen() {
 ...
 <Text className="text-on-surface font-bold text-base text-center mb-4">
 Camera access is needed to {meta.title.toLowerCase()}.
 </Text>
 <Pressable onPress={requestPermission} className="bg-primary px-6 py-3.5 rounded-2xl">
