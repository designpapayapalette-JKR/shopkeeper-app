...
export default function PurchaseHistoryScreen() {
 ...
 renderItem={({ item }) => (
 <Pressable onPress={() => openDetail(item)} className="bg-surface-container-lowest rounded-2xl border border-outline-variant shadow-sm">
 <View className="p-4 flex-row justify-between items-center">
 <View className="flex-1 mr-2">
 <Text className="font-bold text-base text-on-surface ">{item.purchase_number}</Text>
 <Text className="text-sm text-on-surface-variant mt-1">{item.supplier.name} · {formatDate(item.date)}</Text>
 </View>
 <Text className="text-base font-black text-on-surface ">₹{parseFloat(item.grand_total).toLocaleString("en-IN")}</Text>
 </View>
 <View className="border-t border-outline-variant py-2.5 items-center">
 <View className="flex-row items-center" style={{ gap: 5 }}>
 <MaterialCommunityIcons name="chevron-right" size={15} color={theme.colors.onSurfaceVariant} />
 <Text className="text-xs text-on-surface-variant ">{item.items.length} items</Text>
 </View>
 </View>
 </Pressable>
 )}
 ...
 <Text className="text-sm text-on-surface-variant mb-1">Grand Total</Text>
 ...
 <View className="flex-1 mr-2">
 <Text className="text-sm font-bold text-on-surface " numberOfLines={1}>{item.product.name}</Text>
 <Text className="text-xs text-on-surface-variant mt-0.5">
 Qty: {parseFloat(item.quantity).toFixed(0)} × ₹{parseFloat(item.cost).toLocaleString("en-IN")}
 </Text>
 </View>
 <Text className="text-sm font-bold text-on-surface shrink-0">
 ...
 <View key={item.product.id} className="flex-row justify-between items-center bg-surface-container-lowest p-4 rounded-xl border border-outline-variant mb-3">
 ...
 <Text className="text-sm font-semibold text-on-surface-variant uppercase tracking-wider mb-2 mt-2">Reason (optional)</Text>
 <TextInput
 value={returnReason}
 onChangeText={setReturnReason}
 placeholder="e.g. damaged goods, wrong item"
 placeholderTextColor="#A0A0A0"
 className="bg-surface-container-lowest text-on-surface border border-outline-variant rounded-xl px-4 py-4 text-base font-medium mb-8"
 />
