...
function GstDrillDownSection({
 ...
 return (
 <View className="mb-3 bg-surface-container-lowest rounded-xl border border-outline-variant overflow-hidden">
 ...
 {expanded && <View className="border-t border-outline-variant ">{children}</View>}
 </View>
 );
}
...
function GstSaleRowCard({ row }: { row: GstSaleRow }) {
 return (
 <View className="p-3 border-b border-outline-variant last:border-b-0">
 <View className="flex-row justify-between items-center mb-1">
 <Text className="text-sm font-bold text-on-surface ">{row.invoice_number}</Text>
 <Text className="text-sm font-bold text-on-surface ">{formatRupee(row.grand_total)}</Text>
 </View>
...
function GstPurchaseRowCard({ row }: { row: GstPurchaseRow }) {
 ...
 <View className="p-3 border-b border-outline-variant last:border-b-0">
 <View className="flex-row justify-between items-center mb-1">
 <Text className="text-sm font-bold text-on-surface ">{row.purchase_number}</Text>
 <Text className="text-sm font-bold text-on-surface ">{formatRupee(row.grand_total)}</Text>
 </View>
 <Text className="text-xs text-on-surface-variant mb-1">
...
export default function GstReportsScreen() {
 ...
 >
 <Text className={`text-sm font-bold text-center ${activeTab === tab.key ? "text-white" : "text-on-surface "}`}>
 ...
 <Text className="text-sm font-semibold text-on-surface-variant uppercase tracking-wider mb-2">From</Text>
 <TextInput
 value={from}
 onChangeText={setFrom}
 placeholder="YYYY-MM-DD"
 className="bg-surface-container-lowest text-on-surface border border-outline-variant rounded-xl px-4 py-3 text-base"
 />
 </View>
 <View className="flex-1">
 <Text className="text-sm font-semibold text-on-surface-variant uppercase tracking-wider mb-2">To</Text>
 <TextInput
 value={to}
 onChangeText={setTo}
 placeholder="YYYY-MM-DD"
 ...
 {activeTab === "daybook" && (
 <View className="mb-4">
 <Text className="text-sm font-semibold text-on-surface-variant uppercase tracking-wider mb-2">Date</Text>
 <TextInput
 value={dayBookDate}
 ...
 disabled={loading}
 className="bg-primary py-3.5 rounded-xl items-center mb-6"
 >
 ...
 <View>
 {hsnRows.length === 0 ? (
 <Text className="text-on-surface-variant text-center py-8">No GST sales in this range.</Text>
 ) : (
 <>
 {hsnRows.map((row) => (
 <View key={`${row.hsn_code}-${row.tax_rate}`} className="bg-surface-container-lowest p-4 rounded-xl border border-outline-variant mb-3">
 <View className="flex-row justify-between mb-1">
 <Text className="font-bold text-on-surface ">HSN {row.hsn_code}</Text>
 <Text className="font-bold text-on-surface ">{row.tax_rate}%</Text>
 </View>
 ...
 <Text className="text-xs text-on-surface-variant italic p-3">No B2B sales in this range.</Text>
 ) : (
 ...
 <Text className="text-xs text-on-surface-variant italic p-3">No B2C sales in this range.</Text>
 ) : (
 ...
 <Text className="text-xs text-on-surface-variant italic p-3">No purchases in this range.</Text>
 ) : (
 ...
 <Text className="text-on-surface-variant text-sm">Total Inflow</Text>
 <Text className="font-bold text-success text-sm">{formatRupee(dayBook.total_in)}</Text>
 </View>
 <View className="flex-row justify-between mb-1.5">
 <Text className="text-on-surface-variant text-sm">Total Outflow</Text>
 <Text className="font-bold text-error text-sm">{formatRupee(dayBook.total_out)}</Text>
 </View>
 <View className="flex-row justify-between pt-1.5 border-t border-outline-variant ">
 <Text className="text-on-surface-variant font-bold text-sm">Net Balance</Text>
 <Text className={`font-black text-sm ${dayBook.net >= 0 ? "text-primary " : "text-error"}`}>{formatRupee(dayBook.net)}</Text>
 </View>
 </View>

 {/* Action buttons */}
 <Pressable onPress={handleExportDayBook} disabled={exporting} className="border border-primary py-3.5 rounded-xl items-center mb-6">
 {exporting ? <ActivityIndicator color={theme.colors.primary} /> : <Text className="text-primary font-bold text-sm">Export & Share Day Book CSV</Text>}
 </Pressable>
 ...
 {/* Sales Invoices */}
 <View>
 <Text className="text-sm font-bold text-on-surface mb-2 uppercase tracking-wide">Invoices Billed (Sales)</Text>
 {dayBook.invoices.length === 0 ? (
 <Text className="text-xs text-on-surface-variant italic bg-surface-container-lowest/50 p-3 rounded-xl border border-outline-variant ">No sales recorded today.</Text>
 ) : (
 dayBook.invoices.map((i, idx) => (
 <View key={idx} className="bg-surface-container-lowest p-3.5 rounded-xl border border-outline-variant mb-2 flex-row justify-between items-center">
 <View>
 <Text className="font-bold text-on-surface text-sm">{i.invoice_number}</Text>
 <Text className="text-xs text-on-surface-variant ">{i.party_name}</Text>
 </View>
 ...
 {/* Purchases */}
 <View className="mt-4">
 <Text className="text-sm font-bold text-on-surface mb-2 uppercase tracking-wide">Purchases Staged</Text>
 {dayBook.purchases.length === 0 ? (
 <Text className="text-xs text-on-surface-variant italic bg-surface-container-lowest/50 p-3 rounded-xl border border-outline-variant ">No purchases recorded today.</Text>
 ) : (
 dayBook.purchases.map((p, idx) => (
 <View key={idx} className="bg-surface-container-lowest p-3.5 rounded-xl border border-outline-variant mb-2 flex-row justify-between items-center">
 <View>
 <Text className="font-bold text-on-surface text-sm">{p.purchase_number}</Text>
 <Text className="text-xs text-on-surface-variant ">{p.supplier_name}</Text>
 </View>
 ...
 {/* Payments Received */}
 <View className="mt-4">
 <Text className="text-sm font-bold text-on-surface mb-2 uppercase tracking-wide">Payments Received (Inbound)</Text>
 {dayBook.payments_in.length === 0 ? (
 <Text className="text-xs text-on-surface-variant italic bg-surface-container-lowest/50 p-3 rounded-xl border border-outline-variant ">No inbound payments.</Text>
 ) : (
 dayBook.payments_in.map((pi, idx) => (
 <View key={idx} className="bg-surface-container-lowest p-3.5 rounded-xl border border-outline-variant mb-2 flex-row justify-between items-center">
 <View>
 <Text className="font-bold text-on-surface text-sm">{pi.party_name}</Text>
 <Text className="text-xs text-on-surface-variant ">{pi.mode || "Payment"} {pi.reference ? `· ${pi.reference}` : ""}</Text>
 </View>
 ...
 {/* Payments Outflow */}
 <View className="mt-4">
 <Text className="text-sm font-bold text-on-surface mb-2 uppercase tracking-wide">Payments Made (Outbound)</Text>
 {dayBook.payments_out.length === 0 ? (
 <Text className="text-xs text-on-surface-variant italic bg-surface-container-lowest/50 p-3 rounded-xl border border-outline-variant ">No outbound payments.</Text>
 ) : (
 dayBook.payments_out.map((po, idx) => (
 <View key={idx} className="bg-surface-container-lowest p-3.5 rounded-xl border border-outline-variant mb-2 flex-row justify-between items-center">
 <View>
 <Text className="font-bold text-on-surface text-sm">{po.party_name}</Text>
 <Text className="text-xs text-on-surface-variant ">{po.mode || "Payment"} {po.reference ? `· ${po.reference}` : ""}</Text>
 </View>
 ...
 {/* Expenses */}
 <View className="mt-4">
 <Text className="text-sm font-bold text-on-surface mb-2 uppercase tracking-wide">Expenses Recorded</Text>
 {dayBook.expenses.length === 0 ? (
 <Text className="text-xs text-on-surface-variant italic bg-surface-container-lowest/50 p-3 rounded-xl border border-outline-variant ">No expenses recorded today.</Text>
 ) : (
 dayBook.expenses.map((e, idx) => (
 <View key={idx} className="bg-surface-container-lowest p-3.5 rounded-xl border border-outline-variant mb-2 flex-row justify-between items-center">
 <View className="flex-1 pr-2">
 <Text className="font-bold text-on-surface text-sm capitalize">{e.category}</Text>
 {e.notes ? <Text className="text-xs text-on-surface-variant ">{e.notes}</Text> : null}
 </View>
