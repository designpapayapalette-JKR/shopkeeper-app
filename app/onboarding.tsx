...
export default function OnboardingScreen() {
 ...
 {i < STEPS.length - 1 && (
 <View className={`flex-1 h-0.5 ${i < step ? "bg-primary " : "bg-surface-container "}`} />
 )}
 </React.Fragment>
 ))}
 </View>
 ...
 <Text className="text-2xl font-black text-on-surface mb-1">Let&apos;s set up your business</Text>
 ...
 {step === 1 && (
 <ScrollView showsVerticalScrollIndicator={false}>
 <Text className="text-2xl font-black text-on-surface mb-1">Add your team</Text>
 <Text className="text-sm text-on-surface-variant mb-6">
 ...
 placeholderTextColor="#A0A0A0"
 className="text-on-surface border border-outline-variant rounded-xl px-4 py-3.5 text-base font-medium mb-3"
 />
 <TextInput
 value={memberPhone}
 ...
 memberRole === r.id ? "bg-primary border-primary " : "bg-background border-outline-variant "
 }`}
 >
 <Text className={`text-xs font-bold ${memberRole === r.id ? "text-white" : "text-on-surface-variant "}`}>{r.name}</Text>
 </Pressable>
 ))}
 </View>
 <Pressable
 ...
 className="bg-primary/10 py-3 rounded-xl items-center flex-row justify-center"
 style={{ gap: 6 }}
 >
 {addingMember ? (
 ...
 <Text className="text-primary font-bold text-sm">Create Login & Add</Text>
 </>
 )}
 </Pressable>
 </View>
 ...
 <View className="mb-5">
 <Text className="text-sm font-bold text-on-surface-variant uppercase tracking-wider mb-2">
 Added ({addedMembers.length})
 </Text>
 {addedMembers.map((m) => (
 <View
 key={m.email}
 className="bg-surface-container-lowest border border-outline-variant rounded-xl p-3.5 mb-2.5 flex-row items-center justify-between"
 >
 <View className="flex-1 mr-2">
 <Text className="font-bold text-on-surface ">{m.name}</Text>
 <Text className="text-xs text-on-surface-variant mt-0.5">{m.email} · {m.role}</Text>
 </View>
 <Pressable
 ...
 <Text className="text-2xl font-black text-on-surface mb-2 text-center">You&apos;re all set!</Text>
 ...
 className="bg-primary py-4 px-10 rounded-xl items-center"
 >
