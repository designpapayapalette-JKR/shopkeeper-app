...
import { View, Text, FlatList, ActivityIndicator, Pressable, RefreshControl, Modal, ScrollView, Alert, TextInput } from "react-native";
...
function DetailFields({ data }: { data: Record<string, unknown> }) {
 ...
 if (entries.length === 0) {
 return (
 <Text className="text-sm text-on-surface-variant ">No details recorded.</Text>
 );
 }
 return (
 <View style={{ gap: 8 }}>
 ...
 <Text className="text-sm font-semibold text-on-surface-variant " style={{ flex: 1 }}>
 {key}
 </Text>
 <Text className="text-sm text-on-surface " style={{ flex: 1, textAlign: "right" }}>
...
export default function ActivityLogScreen() {
 ...
 <View className="bg-surface-container-lowest border-b border-outline-variant flex-row items-center justify-between px-margin-mobile pb-3" style={{ gap: 12, paddingTop: topInset }}>
 ...
 ListFooterComponent={loadingMore ? <View className="py-4"><ActivityIndicator size="small" color={theme.colors.primary} /></View> : !hasMore && filteredEntries.length > 0 ? <View className="py-4"><Text className="text-center text-sm text-on-surface-variant ">All entries loaded</Text></View> : null}
 ...
 className="bg-surface-container-lowest rounded-xl border border-outline-variant p-md flex-row items-center active:opacity-70"
 style={{ gap: 12 }}
 >
 ...
 {item.notes && (
 <Text className="font-caption text-caption text-on-surface-variant mt-1">
 {item.notes}
 </Text>
 )}
 <Text className="font-caption text-caption text-on-surface-variant mt-1">
 ...
 <Text className="text-xl font-bold text-on-surface " style={{ flex: 1 }}>
 ...
 {detailEntry.notes && (
 <Text className="text-sm text-on-surface-variant mb-3">{detailEntry.notes}</Text>
 )}
 <ScrollView style={{ marginTop: 12 }} contentContainerStyle={{ paddingBottom: 8 }}>
 {detailEntry.detail && "before" in detailEntry.detail ? (
 <View style={{ gap: 16 }}>
 <View>
 <Text className="text-sm font-bold text-on-surface mb-2">Before</Text>
 <DetailFields data={(detailEntry.detail as any).before ?? {}} />
 </View>
 <View>
 <Text className="text-sm font-bold text-on-surface mb-2">After</Text>
