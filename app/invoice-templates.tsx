...
export default function InvoiceTemplatesScreen() {
 ...
 <Text className="text-xs text-on-surface-variant mt-0.5 capitalize">{item.module} · {item.config?.paper_size || "A4"}</Text>
 </View>
 ...
 <Text className="text-xl font-bold text-on-surface ">Invoice Templates</Text>
 </View>
 ...
 className={`px-4 py-3 rounded-xl border ${formModule === m ? "bg-primary border-primary " : "bg-surface-container-lowest border-outline-variant "}`}>
 <Text className={`text-sm font-bold ${formModule === m ? "text-white" : "text-on-surface-variant capitalize"}`}>{m}</Text>
 </Pressable>
 ))}
 </View>
 </View>
 ...
 <Text className="text-sm font-medium text-on-surface flex-1 mr-3">{t.label}</Text>
