...
import { View, ScrollView, ActivityIndicator, RefreshControl, Text, useWindowDimensions } from "react-native";
import { Card, Chip, useTheme, SegmentedButtons } from "react-native-paper";
    ...
    >
      ...
      <View className="flex-row items-center justify-between px-4 mb-4">
        <View className="flex-row items-center" style={{ gap: 8 }}>
          ...
          <Text className="text-2xl font-bold text-on-surface dark:text-text-primary-dark">Analytics</Text>
        </View>
        <SegmentedButtons
          value={months}
          onValueChange={(v) => { setMonths(v); setLoading(true); }}
          buttons={MONTH_OPTIONS}
          density="small"
          style={{ width: 200 }}
        />
      </View>
      ...
      {revenueTrend.length > 0 && (
        <Card mode="elevated" className="mx-4 mb-4">
          <Card.Content>
            <Text className="text-base font-bold text-on-surface dark:text-text-primary-dark mb-3">Revenue & Profit Trend</Text>
            <BarChart
              data={{
            ...
            <View className="flex-row justify-between mt-3" style={{ gap: 8 }}>
              <View className="flex-1 items-center p-2 rounded-xl bg-primary/10">
                ...
                <Text className="text-xs text-on-surface-variant dark:text-text-secondary-dark">Total Revenue</Text>
              </View>
              <View className="flex-1 items-center p-2 rounded-xl" style={{ backgroundColor: "#83540015" }}>
                ...
                <Text className="text-xs text-on-surface-variant dark:text-text-secondary-dark">Total Profit</Text>
              </View>
            </View>
          </Card.Content>
        </Card>
      ...
      {salesByCategory.length > 0 && (
        <Card mode="elevated" className="mx-4 mb-4">
          <Card.Content>
            <Text className="text-base font-bold text-on-surface dark:text-text-primary-dark mb-3">Sales by Category</Text>
            {salesByCategory.slice(0, 5).map((cat, idx) => {
              ...
              return (
                <View key={cat.name} className="flex-row items-center mb-2" style={{ gap: 8 }}>
                  ...
                  <Text className="text-sm text-on-surface dark:text-text-primary-dark flex-1">{cat.name}</Text>
                  <Text className="text-sm font-bold text-on-surface dark:text-text-primary-dark">₹{cat.value.toLocaleString("en-IN")}</Text>
                  <Text className="text-xs text-on-surface-variant dark:text-text-secondary-dark w-10 text-right">{pct.toFixed(0)}%</Text>
                </View>
              );
            })}
          </Card.Content>
      ...
      {paymentModes.length > 0 && (
        <Card mode="elevated" className="mx-4 mb-4">
          <Card.Content>
            <Text className="text-base font-bold text-on-surface dark:text-text-primary-dark mb-3">Payment Modes</Text>
              ...
              {paymentModes.map((pm, idx) => {
                ...
                return (
                  <View key={pm.name} className="flex-1 items-center p-3 rounded-xl" style={{ backgroundColor: `${CHART_COLORS[idx % CHART_COLORS.length]}15` }}>
                    ...
                    <Text className="text-xs font-semibold text-on-surface-variant dark:text-text-secondary-dark mt-1 capitalize">{pm.name}</Text>
                  </View>
                );
              })}
            </View>
      ...
      {topProducts.length > 0 && (
        <Card mode="elevated" className="mx-4 mb-4">
          <Card.Content>
            <Text className="text-base font-bold text-on-surface dark:text-text-primary-dark mb-3">Top Products</Text>
            {topProducts.map((p, idx) => (
              <View key={p.name} className="flex-row items-center py-2" style={{ gap: 10 }}>
                ...
                <View className="flex-1">
                  ...
                  <Text className="text-xs text-on-surface-variant dark:text-text-secondary-dark">{p.quantity} units sold</Text>
                </View>
                <Text className="text-sm font-bold text-on-surface dark:text-text-primary-dark">₹{p.revenue.toLocaleString("en-IN")}</Text>
              </View>
            ))}
          </Card.Content>
        </Card>
      ...
      {inventory && (
        <Card mode="elevated" className="mx-4 mb-4">
          <Card.Content>
            <Text className="text-base font-bold text-on-surface dark:text-text-primary-dark mb-3">Inventory Summary</Text>
            <View className="flex-row flex-wrap" style={{ gap: 8 }}>
              {[
                ...
                <View key={item.label} className="flex-1 min-w-[45%] items-center p-3 rounded-xl" style={{ backgroundColor: `${item.color}10` }}>
                  ...
                  <Text className="text-xs text-on-surface-variant dark:text-text-secondary-dark mt-1">{item.label}</Text>
                </View>
              ))}
            </View>
          </Card.Content>
