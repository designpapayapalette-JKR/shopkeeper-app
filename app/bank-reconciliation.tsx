...
export default function BankReconciliationScreen() {
 ...
 className={`mr-2 px-4 py-3 rounded-lg border ${accountId === a.id ? "bg-primary border-primary" : "bg-surface-container-lowest border-gray-200 "}`}
 >
 <Text className={`text-sm font-semibold ${accountId === a.id ? "text-white" : "text-on-surface-variant "}`}>{a.bankName} — {a.accountNumber}</Text>
 </Pressable>
 ))}
 </ScrollView>
 ...
 <Text className="text-xs text-on-surface-variant mb-4">CSV columns required: date, description, amount (positive = money in, negative = money out).</Text>

 {loading ? (
 <View className="py-10 items-center"><ActivityIndicator color={theme.colors.primary} /></View>
 ) : lines.length === 0 ? (
 <View className="py-10 items-center">
 <Text className="text-sm text-on-surface-variant ">No unmatched statement lines.</Text>
 </View>
 ) : (
 lines.map((l) => (
 <View key={l.id} className="bg-surface-container-lowest p-4 rounded-xl border border-gray-100 mb-3">
 <View className="flex-row justify-between items-start mb-2">
 <Text className="font-bold text-on-surface flex-1 mr-2" numberOfLines={2}>{l.description}</Text>
 <Text className={`font-black ${Number(l.amount) >= 0 ? "text-success" : "text-error"}`}>₹{Number(l.amount).toLocaleString("en-IN")}</Text>
 </View>
 <Text className="text-xs text-on-surface-variant mb-3">{new Date(l.date).toLocaleDateString("en-IN")}</Text>
 ...
 <Pressable onPress={() => ignoreLine(l)} className="flex-1 border border-gray-300 py-2.5 rounded-xl items-center">
 <Text className="text-on-surface-variant font-bold text-sm">Ignore</Text>
 </Pressable>
 </View>
 </View>
 ))
 ...
 <View className="bg-background rounded-t-3xl px-6 pt-6 pb-10" style={{ maxHeight: "75%" }}>
 <View className="flex-row justify-between items-center mb-4">
 <Text className="text-lg font-bold text-on-surface flex-1 mr-2" numberOfLines={1}>{suggestFor?.description}</Text>
 ...
 <Text className="text-sm text-on-surface-variant py-6 text-center">No candidate payments found nearby.</Text>
 ) : (
 <ScrollView>
 {suggestions.map((s) => (
 <Pressable
 ...
 <Text className="font-bold text-on-surface ">{s.payment.party?.name || "—"} — ₹{Number(s.payment.amount).toLocaleString("en-IN")}</Text>
 <Text className={`text-xs font-bold ${s.confidence === "high" ? "text-success" : "text-warning"}`}>{s.confidence}</Text>
 </View>
 <Text className="text-xs text-on-surface-variant mt-0.5">{s.payment.reference || "No reference"} · {s.daysApart.toFixed(0)}d apart</Text>
 </Pressable>
 ))}
 </ScrollView>
 )}
